...
_linode_cli()
{
    ...
    case "${prev}" in
        ...
        users)
            ...
            ;;
        databases)
            COMPREPLY=( $(compgen -W "engines engine-view list mysql-list mysql-create mysql-view mysql-update mysql-delete mysql-backups-list mysql-backup-snapshot mysql-backup-view mysql-backup-restore mysql-creds-view mysql-creds-reset mysql-ssl-cert mysql-patch types type-view --help" -- ${cur}) )
            return 0
            ;;
        domains)
